...
func RegisterUser(name string, password string, email string, phone string) bool {
	...
	if (IsEmail(email) == false) {
		fmt.Println("Error: The Email is error!")
		err = true
	}
	if (IsCellphone(phone) == false) {
		fmt.Println("Error: The Phone is error!")
		err = true
	}
	if (len(password) < 6) {
		fmt.Println("Error: The length of your password should not less than 6!")
		err = true
	}
	_, isExit, _:= Query_user(name)
	if (isExit) {
		fmt.Println("Error: The username has exited, please use another one!")
		err = true
	}
	if (err) {
		return false
...
func Create_meeting(title string, start Date, end Date,sponsor string) bool {
	...
	for i := 0; i < len(mData); i++ {
		if mData[i].Title == title {
			fmt.Println("The meeting has existed!")  
			is_flag = true
		}
	}
	...
	if (len(tmp_m) !=0) {
		for j := 0; j < len(tmp_m); j++ {
			if IsOverlapping(tmp_m[j].Start, tmp_m[j].End, start, end) {
				fmt.Println("The name ",tmp_m[j].Title, " of your meeting has overlaped!")
				is_flag = true
			}
		}
	}
	...
	fmt.Printf("Please input the number of meeting members : ")
	var n int
	var tt string
    fmt.Scanln(&n)
    for i := 0; i< n; i++ {
    	fmt.Scanln(&tt)
    	if tt == sponsor {
    		fmt.Println("You are the sponsor, so you do not have add yourself to meeting member!")
    		os.Exit(1)
    	}
	...
	for k := 0; k < len(m_participator); k++ {
		...
		if (!flag)  {
			fmt.Println(m_participator[k], " is not signed up!")
			is_flag = true
		} else {
			...
			if !flag1 {
				...
				fmt.Println(m_participator[k], " can not attend the meeting, because the time has overlaped!")
				is_flag = true
			} else {
	...
	if count != 0 {
		...
		Meeting_WriteToFile(mData)
		fmt.Println("Create meeting successful!")
		return true
	} else {
		fmt.Println("There is no member! You can not create it!")
		return false
	}
